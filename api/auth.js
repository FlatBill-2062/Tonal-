export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'signup') {
      ...
      // If email confirmation is disabled, Supabase returns a token immediately
      if (data.access_token) {
        ...
        return res.status(200).json({ user: data.user, token: data.access_token, tier: profile.tier, analyses_this_month: profile.analyses_this_month });
      }
      return res.status(200).json({ user: data.user, token: null, message: 'Check your email to confirm your account.' });
    }
